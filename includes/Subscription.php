...
class Subscription {
    ...
    /**
     * Get project limit for a given plan
     * 
     * @param string $planName The plan name (free, pro, enterprise)
     * @return int|null Project limit, or null for unlimited
     */
    public function getProjectLimit($planName) {
        $limits = [
            'free' => 5,
            'pro' => 50,
    ...
    public function canCreateProject() {
        ...
        $limit = $this->getProjectLimit($planName);
        
        // Count current projects
        $currentCount = $this->db->count('projects', 'user_id = :user_id', ['user_id' => $this->userId]);
