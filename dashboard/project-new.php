...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        $projectId = $projectsManager->createProject($user['id'], $title, $description);
        
        if ($projectId) {
            flashMessage('success', 'Project created successfully!');
            redirect('/dashboard/');
        } else {
            flashMessage('error', 'Failed to create project. Please try again.');
        }
    }
}
    ...
    <main class="main-container">
        <?php if (!$canCreate): ?>
            <div style="background: #fef2f2; border: 1px solid #fecaca; border-radius: 12px; padding: 1.5rem; margin-bottom: 1.5rem; color: #991b1b;">
                <h3 style="margin: 0 0 0.5rem 0; font-size: 1.125rem;">Project Limit Reached</h3>
                <p style="margin: 0 0 1rem 0;">You've reached your limit of <?php echo $usage['limit']; ?> projects on the <?php echo ucfirst($usage['plan']); ?> plan.</p>
